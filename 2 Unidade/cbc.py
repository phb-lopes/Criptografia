...
def pipeline(mensagem,chave,text_claro,chave_inicial,op):
	...
	if op == 1:
		...
		tam = len(mensagem)
		aux = mensagem[0]
		# xor = xor_1(mensagem[0],xor)
		
		aux0 = bin(int(str(xor), 			# Tranf. da saida S-box
			scale))[2:].zfill(num_of_bits)
		aux1 = bin(int(str(aux), 			# Tranf. da saida S-box
			scale))[2:].zfill(num_of_bits)	

		xor_bin = xorBinary(aux0,aux1)
		xor_hexa = binToHex(xor_bin)
		
		print(xor_hexa," = ",aux0," ^ [",xor,"]")
		# # xor = (int(str(mensagem[0]),16) ^ int((str(xor)),16))
		# # print(xor)
		...
		if tam > 1:
			while i > 0:
				...
				aux0 = bin(int(str(saida),
					scale))[2:].zfill(num_of_bits)
				aux1 = bin(int(str(mensagem[i-1]),
					scale))[2:].zfill(num_of_bits)	

				xor_bin = xorBinary(aux0,aux1)
				xor_hexa = binToHex(xor_bin)
				
				print(xor_hexa," = ",mensagem[i-1]," ^ [",saida,"]")
				print("=== BLOCO[",i+1,"] - ",mensagem[i])
